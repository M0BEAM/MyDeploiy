...
interface ApplicationUiDeployStaticProps {
  children: ReactNode;
}
export default function ApplicationUiDeployStatic({ children }: ApplicationUiDeployStaticProps) {
  const pathName = usePathname();
  const params = useParams();
  const { id } = params; // Access the dynamic `id` from the URL
  const currentPath = "/dashboard/deploy/static"
  const lastSegment = pathName.split("/").filter(Boolean).pop(); // Get "name"
        ...
        <nav className="px-4 sm:px-6 flex basis-full items-center w-full mx-auto">
          <div className="me-5 lg:me-0 lg:hidden">
            ...
            <a className="flex-none rounded-md text-xl inline-block font-semibold focus:outline-none focus:opacity-80" href="#" aria-label="Preline">
              <Image src={'/assets/logoApp.png'} alt="logo" width={40} height={40} />
            </a>
            {/* <!-- End Logo --> */}
  ...
  dark:bg-neutral-800 dark:border-neutral-700" role="dialog" aria-label="Sidebar">
        <div className="relative flex flex-col h-full max-h-full">
          <div className="px-6 pt-4 flex items-center">
            {/* <!-- Logo --> */}
            <a
              className="flex items-center gap-2 flex-none rounded-xl text-xl font-semibold focus:outline-none focus:opacity-80"
              href="#"
              aria-label="Preline"
            >
              {/* Back arrow SVG */}
             <Link legacyBehavior href={"/dashboard"}>
              ...
              {/* Logo */}
              <Image src="/assets/logoApp.png" alt="logo" width={150} height={150} />
            </a>
          ...
          <div className="h-full overflow-y-auto [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-track]:bg-gray-100 [&::-webkit-scrollbar-thumb]:bg-gray-300 dark:[&::-webkit-scrollbar-track]:bg-neutral-700 dark:[&::-webkit-scrollbar-thumb]:bg-neutral-500">
            <nav className="hs-accordion-group p-3 w-full flex flex-col flex-wrap" data-hs-accordion-always-open>
              <ul className="flex flex-col space-y-1">
                <Link legacyBehavior href={`${currentPath}/${id}/`}>
                  <li>
                    <a className={`${lastSegment === "" && "bg-gray-100"} flex items-center gap-x-3.5 py-2 px-2.5  text-sm text-gray-800 rounded-lg hover:bg-gray-100 focus:outline-none focus:bg-gray-100 dark:bg-neutral-700 dark:text-white`} href="#">
                ...
                <Link legacyBehavior href={`${currentPath}/${id}/logs`}>
                  <li>
                    <a className={`${lastSegment === "logs" && "bg-gray-100"} flex items-center gap-x-3.5 py-2 px-2.5  text-sm text-gray-800 rounded-lg hover:bg-gray-100 focus:outline-none focus:bg-gray-100 dark:bg-neutral-700 dark:text-white`} href="#">
                      <svg className="shrink-0 size-4" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect width="18" height="18" x="3" y="4" rx="2" ry="2" /><line x1="16" x2="16" y1="2" y2="6" /><line x1="8" x2="8" y1="2" y2="6" /><line x1="3" x2="21" y1="10" y2="10" /><path d="M8 14h.01" /><path d="M12 14h.01" /><path d="M16 14h.01" /><path d="M8 18h.01" /><path d="M12 18h.01" /><path d="M16 18h.01" /></svg>
                      Logs
                    </a>
                  </li>
                </Link>
                <Link legacyBehavior href={`${currentPath}/${id}/metrics`}>
                  <li>
                    <a className={`${lastSegment === "metrics" && "bg-gray-100"} flex items-center gap-x-3.5 py-2 px-2.5  text-sm text-gray-800 rounded-lg hover:bg-gray-100 focus:outline-none focus:bg-gray-100 dark:bg-neutral-700 dark:text-white`} href="#">
